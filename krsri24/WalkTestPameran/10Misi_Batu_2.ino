  ...
  if(MIRROR_SIDE == 0){
    ...
    while(state==0){//130
      ...
      if(HeadingJustification(azimuth, set_point, 180, offset, 'n')){
        ...
        compass();
        distance_detection();
        if((front_dis<=13 && (galatArahHitung(set_point, azimuth)>=(-1*offset) && galatArahHitung(set_point, azimuth)<=offset) && (left_dis<=15 && left_dis>0))|| pitch >= 15){
          state=1;
          default_state();
        }
    ...
    while(state==0){//130
      ...
      if(HeadingJustification(azimuth, set_point, 180, offset, 'n')){
        ...
        compass();
        distance_detection();
        if((front_dis<=13 && (galatArahHitung(set_point, azimuth)>=(-1*offset) && galatArahHitung(set_point, azimuth)<=offset) && (right_dis<=15 && right_dis>0))|| pitch < -15){
          state=1;
          default_state();
        }
