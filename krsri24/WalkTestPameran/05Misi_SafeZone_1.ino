void safe_zone1(){
  ...
  if(MIRROR_SIDE==0){
    ...
    while(state==0){
      ...
      compass();
      distance_detection();
      if((front_dis<=28 && front_dis>18) && (right_dis>=11 && right_dis<=16) && (galatArahHitung(set_point, azimuth)>=(-1*offset) && galatArahHitung(set_point, azimuth)<=offset)){
        state=1;
        default_state();
      }
    }
    ...
    while(state==0){
      ...
      compass();
      distance_detection();
      if((front_dis<=28 && front_dis>18) && (left_dis>=11 && left_dis<=16) && (galatArahHitung(set_point, azimuth)>=(-1*offset) && galatArahHitung(set_point, azimuth)<=offset)){
        state=1;
        default_state();
      }
    }
